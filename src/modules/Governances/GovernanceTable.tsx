...
interface TableProps<
  T extends GovernanceColumnType,
  U extends UnmaskingTableType | UsageTableType | StatusTableType
> {
  columns: T[];
  rows: U[];
  tableRole: string;
                  ...
                  {type === 'unmask' && unmaskFunction && (
                    <UIBorderCell>
                      <UICheckbox
                        ...
                        onChange={() => unmaskFunction(row)}
                      />
                    </UIBorderCell>
                  )}
                  {type === 'unmask' && scoreValue !== null && (
                    <UIBorderCell>
                      <UIScorebox>
                        <UIScoreChip
                          label={roundScoreIntelligently(scoreValue)}
                          bgColor={getScoreColor(scoreValue)}
                        />
                      </UIScorebox>
                    </UIBorderCell>
                  )}
                  {columns.map((property, indexProp) => {
                    const { id: propertyId } = property;
                    return (
                      propertyId !== 'unmask' &&
                      propertyId !== 'score' && (
                        <UIBorderCell key={indexProp}>
                          {
