...
const BuildRiskIndicatorTrendListObject = (
  id: string | null,
    ...
    <UIFlexWrapBox key={index} sx={{ gap: 1, alignItems: 'center' }}>
      ...
      >
        ...
        >
          ...
          >
            {index === 0
              ? 'trends with a slope between'
              : 'or trends with a slope between'}
          </Typography>
        </UIFlexWrapBox>
        <UIDefaultTextField
...
const BuildRiskTrendList = ({
  ...
  const orderListValues =
    indicator && indicator?.orderList ? indicator?.orderList.join(', ') : '0';
  return (
    <Box sx={{ mt: 2.5 }}>
      <UIFlexWrapBox>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexDirection: 'column' }}>
          {indicator &&
            ...
            indicator?.rangeList?.map((matchItem, index) => {
              return BuildRiskIndicatorTrendListObject(
                id,
                matchItem,
                index,
                onOpenHistory,
          ...
          {indicator &&
            ...
            BuildRiskIndicatorTrendListObject(
              id,
      ...
      <UIFlexWrapBox sx={{ marginTop: '15px', marginLeft: '535px' }}>
        <Typography
          sx={{ fontSize: '13px', color: '#504F54', paddingTop: '8px' }}
        >
        ...
        <Typography
          sx={{ fontSize: '13px', color: '#504F54', paddingTop: '8px' }}
        >
          with the minimum number of values
        </Typography>
        <UIDefaultTextField
          sx={{ width: '48px' }}
          variant="standard"
          defaultValue={indicator?.instanceCount || 0}
          onChange={(event) =>
            handleInputChange(event, {
              operation: 'changeInstanceCount',
              targetId: id,
            })
          }
          disabled={readOnly}
...
export default BuildRiskTrendList;
