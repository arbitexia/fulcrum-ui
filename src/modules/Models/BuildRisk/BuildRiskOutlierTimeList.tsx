...
const BuildRiskIndicatorOutlierTimeListObject = (
  id: string | null,
    ...
    <UIFlexWrapBox key={index} sx={{ gap: 1, alignItems: 'center' }}>
      ...
      >
        ...
        >
          ...
          >
            {index === 0
              ? 'has a deviation from previous time period of between'
              : 'or between'}
          </Typography>
        </UIFlexWrapBox>
        <UIDefaultTextField
...
const BuildRiskOutlierTimeList = ({
  indicator,
  dataSources,
  riskFields,
  lists,
  ...
  return (
    <Box sx={{ mt: 2.5 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexDirection: 'column' }}>
          {indicator &&
            ...
            indicator?.rangeList?.map((matchItem, index) => {
              return BuildRiskIndicatorOutlierTimeListObject(
                id,
                matchItem,
                index,
                onOpenHistory,
          ...
          {indicator &&
            ...
            BuildRiskIndicatorOutlierTimeListObject(
              id,
      ...
      <UIFlexWrapBox sx={{ mt: 4, gap: 2, alignItems: 'center' }}>
        ...
        />
        <Typography sx={{ fontSize: '13px', color: '#504F54' }}>
          Intraday fill-in number of units
        </Typography>
        <UIDefaultTextField
          sx={{ width: '3em', textAlign: 'right' }}
          inputProps={{ sx: { textAlign: 'right' } }}
          variant="standard"
          defaultValue={indicator?.fillInUnits || 0}
          onChange={(event) =>
            handleInputChange(event, {
              operation: 'changeOutlierFillInUnits',
              targetId: id,
            })
          }
          disabled={readOnly}
...
export default BuildRiskOutlierTimeList;
